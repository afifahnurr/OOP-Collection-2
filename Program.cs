...
namespace campuran_materi_9
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string  tampilanKaryawan()
            {
                int noUrut = 1;
                foreach (Karyawan karyawan in listKaryawan)
                {
                    Console.WriteLine("{0}. NIK: {1}, nama: {2}, Gaji: {3}, {4}", noUrut, karyawan.NIK, karyawan.nama, karyawan.Gaji(), karyawan.JamKerja);
                    noUrut++;
                }
            }

            void tambahKaryawanTetap(string nama, string nik, string jamkerja, double gajibulanan)
            {
                listKaryawan.Add(new KaryawanTetap { nama = nama, NIK = nik, GajiBulanan = gajibulanan, JamKerja = jamkerja });
            }

            void tambahKaryawanHarian(string nama, string nik, string jamkerja, int jumlahjamkerja, int upah)
            {
                listKaryawan.Add(new KaryawanHarian { nama = nama, NIK = nik, JamKerja = jamkerja, JumlahJamKerja = jumlahjamkerja, UpahPerJam = upah });
            }

            void tambahSales(string nama, string nik, string jamkerja, int jumlahpenjualan, int komisi)
            {
                listKaryawan.Add(new Sales { nama = nama, NIK = nik, JamKerja = jamkerja, JumlahPenjualan = jumlahpenjualan, Komisi = komisi });
            }
            ...
            while (keluar == false)
            {
                ...
                else if (menu == 1)
                {
                    ...
                    if (jk == 1)
                    {
                        ...
                        tambahKaryawanTetap(jenis, nik, name, gajibulanan);
                    }
                    else if (jk == 2)
                    {
                        Console.Write("Nik = ");
